...
create policy "fichajes_insert_own_or_admin" on public.fichajes for insert with check (
  ((auth.jwt() ->> 'sub')::uuid = user_id) or exists(select 1 from public.profiles p where p.id = (auth.jwt() ->> 'sub')::uuid and p.rol='admin')
);
...
create policy "fichajes_update_admin" on public.fichajes for update using (
  exists(select 1 from public.profiles p where p.id = (auth.jwt() ->> 'sub')::uuid and p.rol='admin')
) with check (
  exists(select 1 from public.profiles p where p.id = (auth.jwt() ->> 'sub')::uuid and p.rol='admin')
);
